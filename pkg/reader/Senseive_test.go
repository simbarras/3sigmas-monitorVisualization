...
func parseError(records [][]string) bool {
	p := SenseiveParser{}
...
func parseValid(records [][]string, expected []data.Measure) bool {
	p := SenseiveParser{}
	m, _ := p.Parse(records)
...
func TestSenseiveParser_Parse_valid(t *testing.T) {
	...
	if !parseValid(records, expexted) {
...
func FuzzSenseiveParser_ExtractProject_invalid(f *testing.F) {
